...
class AtomImage {
    ...
    addEllipsoid(data, name, parameters = {}) {
        ...
        // Determine the eval ordering convention based on parameters or name
        // We generally don't display coupling tensors, so we can ignore dipolar 
        // and j-coupling conventions
        ...
        // Create a TensorData object from the data
        // This should handle the different input types
        if (!(data instanceof TensorData)) {
            data = new TensorData(data);
        }
    ...
    addEulerDisks(data, name, parameters = {}) {
        ...
        let r = this.renderer;
        if (r) {
            let disks = new r.Primitives.EulerDisks(parameters);
            disks.setTensors(dataA, dataB)
