...
class AtomImage {
    ...
    /** 
     * Index of the crystallographic label of this atom
     * @readonly
     * @type {int}
     */
    get labelIndex() {
        return this._model._label_indices[this._index];
    }
    ...
    addEllipsoid(data, name, parameters = {}) {
        ...
        if (data instanceof Array) {
            ...
            parameters.eigenvalues = data.sorted_eigenvalues('increasing');
            parameters.eigenvectors = data.sorted_eigenvectors('increasing');
    ...
    /**
     * Add an Euler disk to the atom.
     * 
     * @param {Object}                          data   The data to base the
     *                                                 Euler disks on. An Object
     *                                                 with dataA, dataB and angles
     *                                                 members. The former two are
     *                                                 TensorData objects/array or object 
     *                                                 containing the eigenvectors/values of
     *                                                 the tensor. The angles member is an
     *                                                 an array of angles.
     * @param {String}                          name   Name of the Euler disk
     * @param {Object}                          parameters Additional options to
     *                                                    pass (see EulerDisks)
     */
    addEulerDisks(data, name, parameters = {}) {
        this.removeEulerDisks(name);

        parameters = _.clone(parameters); // Avoid editing the reference object
        // extract the eigenvalues and eigenvectors from the data
        parameters.angles = data.angles;
        parameters.conventionA = data.conventionA;
        parameters.conventionB = data.conventionB;
        let dataA = data.dataA;
        let dataB = data.dataB;

        // check if the data is an array or a TensorData object
        if (dataA instanceof Array) {
            // convert to TensorData object
            dataA = new TensorData(dataA);
        }
        
        if (dataB instanceof Array) {
            // convert to TensorData object
            dataB = new TensorData(dataB);
        }
        
        // set the eigenvectors according to chosen convention
        
        parameters.eigenvectors1 = dataA.sorted_eigenvectors(parameters.conventionA);
        parameters.eigenvectors2 = dataB.sorted_eigenvectors(parameters.conventionB);

        // // transpose the eigenvectors? 
        // parameters.eigenvectors1 = mjs.transpose(parameters.eigenvectors1);
        // parameters.eigenvectors2 = mjs.transpose(parameters.eigenvectors2);
        ...
        var r = this.renderer;
        if (r) {
            var disks = new r.Primitives.EulerDisks(parameters);
            this._euler_disks[name] = disks;
            this.mesh.add(disks);
        }
    }

    /**
     * Remove the Euler disk with a given name
     * 
     * @param {String}  name     Name of the Euler disk
     * @param {Object}  parameters Additional options to pass (see EulerDisks)
     * @param {String}  name   Name of the Euler disk
     * 
     */
    removeEulerDisks(name) {
        let l = this._euler_disks[name];
        if (l)
            this._mesh.remove(l);
        delete this._euler_disks[name];
    }

    /**
     * Retrieve or set an Euler disk's properties
     * 
     * @param {String}  name     Name of the Euler disk
     * @param {String}  property Property to set
     * @param {?}       value    Value to set. If omitted, returns the current
     ...
     * 
     * @example
     * atom.eulerDiskProperty('myDisk', 'color', 0xff0000);
     * 
     */
    eulerDisksProperty(name, property, value = null) {
        if (value) {
            this._euler_disks[name][property] = value;
        } else {
            return this._euler_disks[name][property];
        }
    }
