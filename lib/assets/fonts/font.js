...
function parseInlineXmlFont(fntfile) {

    if (!fntfile.startsWith('data:text/xml')) {
        return null;
    }

    try {
        const payload = fntfile.slice(fntfile.indexOf(',') + 1);
        return parseXML(decodeURIComponent(payload));
    } catch {
        return null;
    }
}
...
class BitmapFont {
    ...
    constructor(fntfile, pngfile) {
        ...
        const obj = this;
        this.font = parseInlineXmlFont(fntfile);
        this.texture = null;
