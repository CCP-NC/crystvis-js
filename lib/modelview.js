...
class ModelView {
    ...
    /**
     * Add euler disks to the atom images in this ModelView
     * 
     * @param {Object | Object[] | Function}    data    Data to use for the euler disk
     *                                                  (see AtomImage.addEuler for details)
     * @param {String | String[] | Function}    name    Name of the euler disk
     * @param {Object | Object[] | Function}    args    Arguments for creating the euler disk
     */
    addEulerDisks(data, name = 'euler', args = {}) {
            ...
            for (var i = 0; i < this.length; ++i) {
                var aimg = this._images[i];
                aimg.addEulerDisks(data[i], name[i], args[i]);
            }
    ...
    /**
     * Remove euler disks from the atom images in this ModelView
     * 
     * @param {String | String[] | Function}    name    Name of the euler disk to remove
     * 
     * @return {ModelView}    This ModelView
     * 
     * @example
     * 
     * // Remove all euler disks
     * modelView.removeEulers();
     * 
     **/
    removeEulerDisks(name = 'euler') {
                
        name = this._standardValueArray(name);

        for (var i = 0; i < this.length; ++i) {
            var aimg = this._images[i];
            aimg.removeEulerDisks(name[i]);
        }
    ...
    /**
     * Get or set euler disks' properties for the atom images in this ModelView
     * 
     * @param {String | String[] | Function}    name        Name of the euler disk
     * @param {String | String[] | Function}    property    Property to get or set
     * @param {Any | Any[] | Function}          value       If not provided, get. If provided,
     *                                                     set this value
     * 
     * @return {Any | Any[]}    If value is not provided, return the property values
     *                         If value is provided, return this ModelView
     * 
     * @example
     * 
     * // Get the color of all euler disks
     * var colors = modelView.eulerProperties('color');
     * 
     * // Set the color of all euler disks
     * modelView.eulerProperties('color', 'red');
     * 
     * // Set the color of all euler disks to be different
     * modelView.eulerProperties('color', ['red', 'green', 'blue']);
     * 
     **/
    eulerProperties(name = 'euler', property = 'color', value = null) {
        ...
        for (var i = 0; i < this.length; ++i) {
            var aimg = this._images[i];
            if (value !== null) {
                aimg.eulerDisksProperty(name[i], property[i], value[i]);
            } else {
                ans.push(aimg.eulerDisksProperty(name[i], property[i]));
            }
        }
