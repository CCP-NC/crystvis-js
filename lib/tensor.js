...
import { floatEqual } from './utils.js';
...
class TensorData {
     ...
     * 
     * @return {Array}             Array of [alpha, beta, gamma] in degrees
     * 
     * TODO: check edge cases, esp. axial symmetry 
     * TODO: add in conversion from radians to degrees
     */
    euler(mode = 'zyz', active = true, eval_convention = null) {
    ...
    equivalentEuler(mode = 'zyz', active = true, eval_convention = null) {
        let eulers = this.euler(mode, active, eval_convention);
        return equivalentEuler(eulers, mode, active);
    }
     ...
     * @param {boolean} [passive=false] - Whether to use passive rotation. Default is false.
     ...
     */
    eulerTo(otherTensor, convention = 'zyz', passive = false, eps = 1e-6) {
        ...
        return this._matrixToEuler(mjs.transpose(R), convention, passive);
    }
...
function equivalentEuler(eulerAngles, convention = "zyz", active = true) {
