...
import { floatEqual, arraysAlmostEqual, tensorDataToMatrix3, matrix3ToTensorData, string2vector, rotate_matrix, tryallanglestest } from './utils.js';
...
class TensorData {
    ...
    euler(mode = 'zyz', active = true, eval_convention = null, degrees = false, EPS = 1e-6) {
        ...
        //  Remove this for now as I've moved it into the eulerFromU function
        // if (!active) {
        ...
        [alpha, beta, gamma] = eulerFromU(U, mode, active, EPS);

        let M = rotateTensor(alpha, beta, gamma, PAS, mode, active).toArray();
        // U PAS U^-1
    ...
    equivalentEuler(mode = 'zyz', active = true, eval_convention = null, degrees = false) {
        ...
        return equivalentEuler(eulers, active, degrees);
    }

    rotationTo(otherTensor) {
        // wrapper around the rotationTo function
        return rotationTo(mjs.transpose(this.eigenvectors), mjs.transpose(otherTensor.eigenvectors));
    }
     ...
     *
     * @returns {Array} The Euler angles [alpha, beta, gamma] in radians that rotate this tensor to the other tensor.
     *
    ...
    eulerTo(otherTensor, convention = 'zyz', active = true, eps = 1e-6) {
        ...
        // first make sure they're not the same within eps
        // check evals and evecs are the same within eps
        if (mjs.norm(mjs.subtract(Aevals, Bevals)) < eps) {
            // check eigenvectors are the same up to a sign, within eps
            if (
                arraysAlmostEqual(Aevecs, Bevecs, eps) ||
                arraysAlmostEqual(Aevecs, mjs.multiply(-1, Bevecs), eps) ||
                arraysAlmostEqual(mjs.multiply(-1, Aevecs), Bevecs, eps)
            ){
        ...
        if (this.symmetry == 1 || otherTensor.symmetry == 1) {
            ...
            // Do we need to make sure the rotation matrices are normalised?
            let Rrel1 = mjs.multiply(mjs.inv(Bevecs), Aevecs);
            let Rrel2 = mjs.multiply(mjs.inv(Aevecs), Bevecs);
            ...
            } // end of both symmetric
            else if (this.symmetry == 1 && otherTensor.symmetry == 0) {
                // if the first tensor is axially symmetric and the second is not
                // just get the angles from Rel1
                let eulerAngles = eulerFromU(Rrel1, convention, true); // enforce active here
                eulerAngles = handleEulerEdgeCases(eulerAngles, Aevals, this.symmetric, convention, active, eps);
                if (!active) {
                    // if active, we need to swap the angles
                    eulerAngles = [eulerAngles[2], eulerAngles[1], eulerAngles[0]];
                }
                return eulerAngles;
            }
            else if (this.symmetry == 0 && otherTensor.symmetry == 1) {
                // if the second tensor is axially symmetric and the first is not
                
                // TODO: Question: in my tests I find that if A is axially symmetric and B is not
                // then A->B ZYZ active = B->A ZYZ passive and vice versa (same for ZXZ). 
                // Do we then need this code path? I'll leave it here for now, for easier comparison to 
                // the code in TensorView.
                
                let eulerAngles = eulerFromU(Rrel2, convention, false); // always passive
                const pas1 = mjs.diag(Aevals);
                const pasv2 = mjs.diag(Bevals);
                // TODO: check if we need to re-order Rrel2 to match ordering convention chosen for other tensor
                eulerAngles = tryallanglestest(eulerAngles, pas1, pasv2, Rrel1, convention)
                // reverse and negate the angles
                eulerAngles = [-eulerAngles[2], -eulerAngles[1], -eulerAngles[0]];
                eulerAngles = handleEulerEdgeCases(
                    eulerAngles,
                    Bevals,
                    otherTensor.symmetric,
                    convention,
                    active,
                    eps);
                // if active, we need to swap the angles back
                if (active) {
                    eulerAngles = [eulerAngles[2], eulerAngles[1], eulerAngles[0]];
                }
                return eulerAngles;
        ...
        let R = rotationTo(mjs.transpose(Aevecs), mjs.transpose(Bevecs));
        ...
        let eulerAngles = eulerFromU(R, convention, active);
        eulerAngles = normaliseEulerAngles(eulerAngles, active, eps);
        return eulerAngles;
    }

        /**
     * Calculates the Equivalent Euler angles that rotate this tensor to another tensor.
     *
     * @param {TensorData} otherTensor - The tensor to which we want to rotate.
     * @param {string} [convention='zyz'] - The convention to use for the Euler angles. Default is 'zyz'.
     * @param {boolean} [active=true] - Whether to use active rotation or passive. Default is true (active).
     * @param {number} [eps=1e-6] - The tolerance for considering two tensors as identical. Default is 1e-6.
     * @param {boolean} [degrees=false] - Whether to return the angles in degrees. Default is false.
     *
     * @returns {Array} Array of the 16 equivalent Euler angles [alpha, beta, gamma] in radians that rotate this tensor to the other tensor.
     *
     *
     * @warns If the tensors are identical or if the Euler angles are ambiguous for degenerate tensors.
     */
    equivalentEulerTo(otherTensor, convention = 'zyz', active = true, eps = 1e-6, degrees = false) {
        let eulers = this.eulerTo(otherTensor, convention, active, eps);
        return equivalentRelativeEuler(eulers, active, degrees);
    }
...
function handleEulerEdgeCases(eulerAngles, eigenvalues, originalTensor, mode = "zyz", active = false, eps = 1e-6) {
    ...
    // this is the tricky one - doubly degenerate (axial symmetry )
        ...
        // Make sure we're in the canonical range:
        eulerAngles = normaliseEulerAngles(eulerAngles, active)
    }
...
/**
 * Calculates equivalent relative Euler angles between two tensors.
 * In the general case, there should be 16 sets of equivalent relative Euler angles.
 *
 * @param {Array} eulerAngles - An array of three Euler angles [alpha, beta, gamma] in **radians**.
 * @param {boolean} [active=true] - Whether the rotation is active or passive.
 * @param {boolean} [degrees=false] - Whether to return the angles in degrees. If false, the angles are returned in radians.
 *
 * @returns {Array} An array of 16 sets of equivalent Euler angles. Each set of angles is an array of three floats.
 *
 * The first set of angles is the original Euler angles. The remaining sets are calculated based on the convention and whether the rotation is active or passive.
 *
 * If degrees is true, all angles are converted to degrees.
 */
function equivalentRelativeEuler(eulerAngles, active = true, degrees = false) {
    let equivAngles = mjs.zeros([16, 3]);
    let [alpha, beta, gamma] = eulerAngles;

    if (active) {
        equivAngles = [
            [alpha, beta, gamma],
            [alpha + PI, PI - beta, 2*PI - gamma],
            [alpha + PI, PI - beta, PI - gamma],
            [alpha, beta, PI + gamma],
            [2*PI - alpha, PI - beta, PI + gamma],
            [PI - alpha, beta, PI - gamma],
            [PI - alpha, beta, 2*PI - gamma],
            [2*PI - alpha, PI - beta, gamma],
            [PI - alpha, PI - beta, PI + gamma],
            [2*PI - alpha, beta, PI - gamma],
            [2*PI - alpha, beta, 2*PI - gamma],
            [PI - alpha, PI - beta, gamma],
            [alpha + PI, beta, gamma],
            [alpha, PI - beta, 2*PI - gamma],
