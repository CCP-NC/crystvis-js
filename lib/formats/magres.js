...
function load(contents, filename='magres') {
    ...
    let is_magres_old = 0;                  // 0 = Undecided, 1 = yes, -1 = no
    let block_name = null;
    let block_lines = [];
    ...
    for (let i = 1; i < lines.length; ++i) {
        ...
        if (b_alt_match) {
            if (is_magres_old >= 0) {
                ...
                throw Error('Old style Magres tag found in new style file');
            }
        }
        else if (b_match) {
            if (is_magres_old <= 0) {
                ...
                throw Error('New style Magres tag found in old style file');                
            }
        }
