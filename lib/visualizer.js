...
class CrystVis {
     ...
     * 
     * @param  {String|Object} contents  The contents of the structure file. For the
     *                                    `magres-json` format, a parsed JSON object is also accepted.
     * @param  {String} format      The file's format (cif, xyz, magres, magres-json, etc.). Default is cif.
    ...
    loadModels(contents, format = 'cif', prefix = null, parameters = {}) {
        ...
        for (var n in structs) {
            ...
            this._model_sources[nn]     = { text: typeof contents === 'string' ? contents : JSON.stringify(contents), extension: format };
    ...
    /**
     * Public render method for compatibility with demo code and external calls.
     */
    render() {
        this._renderer._render();
    }
