...
class EulerDisks extends THREE.Group {
    ...
    _updateArcs() {
        ...
        this._updateArcGeometry(this.alphaArc, alphaPoints);
        this._updateArcGeometry(this.betaArc, betaPoints);
        this._updateArcGeometry(this.gammaArc, gammaPoints);
        
        // Update labels
        this._updateArcLabels(angles, alphaPoints, betaPoints, gammaPoints);
    }

    _createArcPoints(axis, angle, startVec, radius, steps = 32) {
        const points = [];
