...
class TextSprite extends THREE.Mesh {
    ...
    set color(c) {
        ...
        if (typeof c === 'number') {
            ...
            const hexString = '#' + ('000000' + c.toString(16)).slice(-6);
            this.material.uniforms.color.value.set(hexString);
        } else {
    ...
    get height() {
        // geo.layout.height is fixed at construction; derive back from targScale
        return this.material.uniforms.targScale.value * this.geometry.layout.height;
    }

    set height(h) {
        const s = h / this.geometry.layout.height;
        this.material.uniforms.targScale.value = s;
        this.material.uniformsNeedUpdate = true;
        // Update mesh scale for non-fixScale sprites (faceCamera=false)
        if (!this.material.uniforms.fixScale.value) {
            this.scale.set(s, -s, s);
        }
    }
