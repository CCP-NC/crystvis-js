...
import { decompose_rotation } from 'nmrtensor-wasm';
    ...
    _buildConfigurations() {
        ...
        for (const sourceTransform of FRAME_TRANSFORMS) {
            ...
            for (const targetTransform of FRAME_TRANSFORMS) {
                ...
                // Relative rotation matrix R = source^T * target
                const relativeRotation = mjs.multiply(mjs.transpose(source), target);
                const relFlat = relativeRotation.toArray ? relativeRotation.toArray().flat() : relativeRotation._data.flat();

                // Call Rust WASM decompose_rotation directly on the relative rotation matrix
                let [alpha, beta, gamma] = Array.from(
                    decompose_rotation(relFlat, sequence, active)
                );
                ...
                if (isSingular && !srcAxial && !tgtAxial) {
                    if (this.specification.singularGauge === 'alpha-zero') {
                        const zAlignment = array(relativeRotation)[2][2];
