...
class RelativeTensorOrientation {
    constructor(source, target, specification = {}) {
        ...
        this.orientationClass = this.sourceClass === 'spherical' || this.targetClass === 'spherical'
            ? 'indeterminate'
            : this.sourceClass === 'axial' || this.targetClass === 'axial'
                ? 'continuous'
                : 'discrete';
        this.freeRotation = this.orientationClass === 'continuous'
            ? {
                domain: [0, 2 * PI],
                source: this.sourceClass === 'axial' ? this.sourceSymmetry : null,
                target: this.targetClass === 'axial' ? this.targetSymmetry : null
            }
            : null;
    ...
    _buildConfigurations() {
        ...
        for (const sourceTransform of FRAME_TRANSFORMS) {
            const source = mjs.multiply(sourceFrame, sourceTransform.matrix);
            for (const targetTransform of FRAME_TRANSFORMS) {
                const target = mjs.multiply(targetFrame, targetTransform.matrix);
                const activeRotation = mjs.multiply(target, mjs.transpose(source));
                const passiveRotation = mjs.transpose(activeRotation);
                const rotation = this.specification.active ? activeRotation : passiveRotation;
                const decompositionRotation = this.specification.active ? activeRotation : passiveRotation;
                const solution = decomposeActive(
                    this.specification.active ? decompositionRotation : mjs.transpose(decompositionRotation),
                    this.specification.sequence,
                    this.rotationTolerance,
                    this.specification.singularGauge
                );
                const reconstructed = elementaryRotation(
                    ...solution.angles,
                    this.specification.sequence,
                    this.specification.active
                );
                if (mjs.norm(mjs.subtract(rotation, reconstructed), 'fro') > this.rotationTolerance * 10) {
                ...
                configurations.push({
                    ...
                    passiveRotation: array(passiveRotation),
                    rotation: array(rotation),
                    euler: solution.angles,
