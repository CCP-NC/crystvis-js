...
class Renderer {
    ...
    */
    ...
    // Convenience getters / setters that update one theme slot at a time without
    // requiring the caller to reconstruct the full theme object.
     ...
     */
    get background() {
        return this._theme ? this._theme.background : null;
    }
    ...
    /**
     * Highlight / aura colour stored inside the active theme.
     * Downstream code (e.g. custom shaders, downstream apps) reads this
     * from `renderer.theme.highlight`.
     * @type {number}
     */
    get highlightColor() {
        return this._theme ? this._theme.highlight : null;
    }

    set highlightColor(c) {
        const hex = new THREE.Color(c).getHex();
        this._theme = Object.assign({}, this._theme, { highlight: hex });
    }

    /**
     * Default colour used for newly-created atom text labels.
     * @type {number}
     */
    get labelColor() {
        return this._theme ? this._theme.label_color : null;
    }
    ...
    /**
     * Colour of the unit-cell box wireframe.
     * Note: changes take effect for newly-displayed models; models that are
     * already visible must be re-rendered (e.g. via `reloadModel()`) to pick
     * up the new colour.
     * @type {number}
     */
    get cellLineColor() {
        return this._theme ? this._theme.cell_line_color : null;
    }
